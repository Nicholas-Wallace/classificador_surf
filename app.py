...
# ==================================================================
#                   funcao predict para gradio
# ==================================================================
def predict_video(video_path):
    """
    Takes a video file path, processes it, and returns classification probabilities.
    """
    # 1. Process the video using your existing function
    video_frames = frames_from_video_file(video_path, n_frames=10) #
    
    # 2. Add the batch dimension for the model
    model_input = np.expand_dims(video_frames, axis=0) #

    # 3. Get predictions (logits) from the model
...
# 5. Format the output for Gradio's Label component
    return {CLASS_NAMES[0] : probabilities[0],
...
with gr.Blocks() as demo:
    gr.Markdown("# Classificador de Manobras 🎬")
    gr.Markdown("Coloque sua manobra que o modelo dira qual foi ('aerial' or 'rasgada').")
    
    with gr.Row():
        video_input = gr.Video(label="Upload Video")
        output_label = gr.Label(num_top_classes=2, label="Prediction")

    video_input.upload(fn=predict_video, inputs=video_input, outputs=output_label)
